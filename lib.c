...
bool _insert_element_child(Node* node, const Point point, const size_t size_props, void* element, const int split_case)
{
	if( split_case % 2 == 0) {
...
void* _get_element_child(const Node* node, const Point* point,const size_t size_props, const int split_case)
{
	if(_same_point(&node->point, point)) {
		...
		if(split_case % 2 == 0) {
			...
			if(node->point.y < point->y) {
				if(node->right != NULL) {
					return _get_element_child(node->right, point, size_props, split_case + 1); } else {
					return NULL;
				}
			} else {
				if(node->left != NULL) {
